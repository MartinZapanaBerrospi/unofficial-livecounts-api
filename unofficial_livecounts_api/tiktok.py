...
class TiktokAgent:
    ...
    def find_user(query: str) -> list[TiktokUser]:
        """Search for TikTok users based on a username query (Synchronous)."""
    ...
    async def find_user_async(query: str) -> list[TiktokUser]:
        """Search for TikTok users based on a username query (Asynchronous)."""
        raw_users = await async_send_request(f"{env.TIKTOK_USER_SEARCH_API}/{query}")
        return [
            TiktokUser(
                user_id=item.get("userId", ""),
                username=item.get("id", ""),
    ...
    def fetch_user_metrics(query: str) -> TiktokUserCount:
        """Fetch engagement metrics and statistics for a TikTok user (Synchronous)."""
    ...
    @staticmethod
    async def fetch_user_metrics_async(query: str) -> TiktokUserCount:
        """Fetch engagement metrics and statistics for a TikTok user (Asynchronous)."""
        metrics = await async_send_request(f"{env.TIKTOK_USER_STATS_API}/{query}")
        return TiktokUserCount(
            user_id=query,
            follower_count=metrics.get("followerCount", 0),
            like_count=metrics.get("likeCount", 0),
    ...
    def find_video(query: str) -> TiktokVideo:
        """Find a TikTok video by its URL or video ID (Synchronous)."""
        if validators.url(query):
            query = TiktokAgent.__extract_video_id_from_given_url(query)
        return TiktokAgent.__find_video_by_id(query)

    @staticmethod
    async def find_video_async(query: str) -> TiktokVideo:
        """Find a TikTok video by its URL or video ID (Asynchronous)."""
        if validators.url(query):
            query = TiktokAgent.__extract_video_id_from_given_url(query)
        return await TiktokAgent.__find_video_by_id_async(query)
    ...
    @staticmethod
    async def __find_video_by_id_async(video_id: str) -> TiktokVideo:
        video = await async_send_request(url=f"{env.TIKTOK_VIDEO_SEARCH_API}/{video_id}")
        user = video.get("author", {})
        return TiktokVideo(
            video_id=video_id,
            title=video.get("title", ""),
    ...
    def fetch_video_metrics(query: str) -> TikTokVideoCount:
        """Fetch engagement metrics for a TikTok video (Synchronous)."""
        if validators.url(query):
            query = TiktokAgent.__extract_video_id_from_given_url(query)
    ...
    @staticmethod
    async def fetch_video_metrics_async(query: str) -> TikTokVideoCount:
        """Fetch engagement metrics for a TikTok video (Asynchronous)."""
        if validators.url(query):
            query = TiktokAgent.__extract_video_id_from_given_url(query)
        metrics = await async_send_request(f"{env.TIKTOK_VIDEO_STATS_API}/{query}")
        return TikTokVideoCount(
            video_id=query,
            like_count=metrics.get("likeCount", 0),
            comment_count=metrics.get("commentCount", 0),
