...
CSS = (
    ...
    ".search-card{background:rgba(15,23,42,0.8);border:1px solid rgba(255,255,255,0.08);border-radius:28px;padding:10px;display:flex;align-items:center;gap:12px;max-width:960px;margin:0 auto;box-shadow:0 30px 60px rgba(0,0,0,0.5);backdrop-filter:blur(30px)}"
    ".elite-btn{background:linear-gradient(135deg,#6366f1,#a855f7)!important;border:none!important;border-radius:20px!important;color:white!important;font-weight:900!important;padding:12px 32px!important;box-shadow:0 8px 16px rgba(99,102,241,0.2)!important;height:58px!important}"
    ...
    ".digit-cell{display:inline-flex;align-items:center;justify-content:center;width:clamp(50px,8vw,85px);height:clamp(80px,13vw,135px);background:#0f172a;border:1px solid rgba(255,255,255,0.15);border-radius:clamp(16px,2vw,32px);font-size:clamp(3.5rem,6.5vw,8rem);font-weight:900;color:#f8fafc;font-variant-numeric:tabular-nums;box-shadow:inset 6px 6px 12px #01040a,inset -6px -6px 12px #030a1c,0 20px 40px rgba(0,0,0,0.8)}"
    ...
    ".icon-3d{font-size:3rem!important;background:linear-gradient(135deg,#6366f1,#a855f7);-webkit-background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 4px 10px rgba(0,0,0,0.5))}"
    "@media(max-width:768px){.search-card{flex-direction:column;padding:12px}.platform-select{width:100%!important;border-right:none!important;border-bottom:1px solid rgba(255,255,255,0.1)!important}}"
)
...
def fetch_live_data(uid, pk):
    ...
    except Exception as e: return {"error": f"Sync Error: {e}"}
...
if st.session_state.show_results and not st.session_state.user_id:
    st.markdown('<div class="main-container"><h1 style="color:white;text-align:center;margin-bottom:3rem;font-weight:900;">RESULTADOS Hallados</h1>', unsafe_allow_html=True)
    if not st.session_state.search_results:
        st.warning("No se hallaron resultados."); st.button("⬅️ VOLVER", on_click=clear_all); st.rerun()
    else:
        grid = st.columns(3)
        for i, r in enumerate(st.session_state.search_results):
            with grid[i % 3]:
                ...
                if st.button(f"SINCRONIZAR", key=f"s_{i}", use_container_width=True, type="primary"):
    ...
    st.button("⬅️ CANCELAR", use_container_width=True, on_click=clear_all); st.markdown('</div>', unsafe_allow_html=True)
    ...
    with c1: st.markdown(f'<div style="color:#64748b;font-weight:900;letter-spacing:4px;font-size:1rem;">⚡ ELITE LIVE FEED</div>', unsafe_allow_html=True)
    with c2: st.button("🔄 BUSCAR OTRO", type="primary", on_click=clear_all)
    ...
    fig = go.Figure(); fig.add_trace(go.Scatter(x=st.session_state.history_times, y=st.session_state.history_values, mode='lines', line=dict(color=pinfo["color"], width=6, shape='spline'), fill='tonexty', fillcolor=f"rgba({int(pinfo['color'][1:3],16)},{int(pinfo['color'][3:5],16)},{int(pinfo['color'][5:7],16)},0.1)", showlegend=False))
